...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-black flex items-center justify-center p-6 selection:bg-[#a31621]/20">
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none opacity-[0.05] overflow-hidden -z-10">
        <svg className="absolute -top-32 -right-32 w-[40rem] h-[40rem] text-[#a31621]" fill="currentColor" viewBox="0 0 100 100"><circle cx="50" cy="50" r="45" /></svg>
        <svg className="absolute -bottom-32 -left-32 w-[40rem] h-[40rem] text-[#a31621]" fill="currentColor" viewBox="0 0 100 100"><circle cx="50" cy="50" r="35" /></svg>
      </div>

      <div className="w-full max-w-md fade-in">
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center p-5 bg-[#a31621]/10 rounded-full mb-8">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-[#a31621]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <p className="text-[10px] uppercase tracking-[0.3em] text-stone-400 dark:text-stone-600 font-bold">Enter to preserve daily updates</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-stone-900/40 backdrop-blur-xl p-10 rounded-[3rem] border border-stone-200 dark:border-stone-800 shadow-2xl space-y-8">
          <div className="space-y-6">
            <div>
              ...
              <input 
                ...
                className="w-full bg-stone-50 dark:bg-stone-800/50 border border-stone-100 dark:border-stone-800 rounded-2xl px-6 py-4 focus:ring-1 focus:ring-[#a31621]/30 transition-all outline-none text-stone-700 dark:text-stone-200 text-sm"
                placeholder=""
          ...
          <button 
            ...
            className={`w-full py-5 rounded-2xl text-[11px] uppercase tracking-[0.3em] font-bold transition-all duration-500 shadow-xl
              ${isLoggingIn 
                ? 'bg-[#a31621]/20 text-[#a31621] cursor-not-allowed' 
                : 'bg-stone-800 dark:bg-[#a31621] text-white dark:text-stone-900 hover:scale-[1.02] active:scale-[0.98] shadow-[#a31621]/10'
              }`}
          >
            {statusText}
          </button>
        ...
        <div className="mt-12 text-center">
           ...
           <p className="text-[10px] uppercase tracking-[0.5em] text-stone-400 dark:text-stone-700">Hari Charan Katta &bull; {new Date().getFullYear()}</p>
        </div>
      </div>
    </div>
  );
